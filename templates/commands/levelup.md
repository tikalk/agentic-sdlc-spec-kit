...
1. Run `{SCRIPT}` from the repo root and parse JSON for `FEATURE_DIR`, `BRANCH`, `SPEC_FILE`, `PLAN_FILE`, `TASKS_FILE`, `RESEARCH_FILE`, `QUICKSTART_FILE`, `TRACE_FILE`, `KNOWLEDGE_ROOT`, and `KNOWLEDGE_DRAFTS`. All file paths must be absolute.
...
2. Load the implementation artifacts:
     ...
     - **Check for session trace**: Read `FEATURE_DIR/trace.md` if exists (generated by `/trace` command).
       ...
       - If trace missing: Continue without trace (optional artifact). Synthesize session context from spec, plan, and tasks_meta.json only.
