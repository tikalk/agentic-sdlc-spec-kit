...
> **Retirement note (`0.14.4+adlc1`)**: The former leaf module `_hooks_fork.py` (agent runtime hooks: `HookAdapter` ABC + Claude/Cursor/Codex/opencode adapters, bridge script, `resolve_hooks`/`install_integration_hooks`/`remove_integration_hooks`) was **deleted** in favor of upstream `events.py` (PR #3704, authored by the fork maintainer). The upstream rework renames `runtime_hooks:` → `events:`, uses snake_case canonical names, folds adapters into integration class attributes (`CANONICAL_TO_NATIVE`/`events_config_file`/`events_format`), and adds the `specify event run` dispatcher. All 8 event-capable integrations now use upstream class attributes instead of fork `--hooks` option. The fork's `agent-context` extension migrated its `runtime_hooks: SessionStart` → `events: session_start`.
