...
if $JSON_MODE; then
    ...
    printf '{"REPO_ROOT":"%s","CDR_FILE":"%s","TEAM_DIRECTIVES":"%s","TEAM_DIRECTIVES_EXISTS":%s,"SKILLS_DRAFTS":"%s","BRANCH":"%s",%s}\n' \
        "$REPO_ROOT" "$CDR_FILE" "$TEAM_DIRECTIVES" "$TEAM_DIRECTIVES_EXISTS" "$SKILLS_DRAFTS" "$CURRENT_BRANCH" "${subsystem_data:1}"
else
