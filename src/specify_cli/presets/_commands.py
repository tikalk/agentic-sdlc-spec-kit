...
def preset_list():
    ...
    # keeps printed order aligned with how presets are composed/resolved.
    installed = sorted(
        installed,
        key=lambda pack: (pack.get("priority", 10), str(pack.get("id", ""))),
    )

    console.print(f"\n{accent('Installed Presets (in resolution order — highest precedence first):', bold=True)}\n")
...
def preset_add(
    ...
    try:
        if dev:
            ...
            console.print(f"Installing preset from {accent(_escape_markup(from_url))}...")
            import urllib.error
            import tempfile
...
def preset_info(
    ...
    if local_pack:
        console.print(
            f"\n{accent(f'Preset: {_escape_markup(str(local_pack.name))}', bold=True)}\n"
        )
    ...
    console.print(f"\n{accent(f'Preset: {name}', bold=True)}\n")
    ...
    console.print(f"  Install with: {accent(f'specify preset add {safe_preset_id}')}")
    console.print()
