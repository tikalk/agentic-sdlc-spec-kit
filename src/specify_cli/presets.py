...
class PresetManager:
    ...
    def refresh_preset_commands(self, preset_name: str) -> None:
        """Re-register commands for an already-installed preset.

        Used by 'preset update' to ensure 'replaces' logic is applied
        (removes superseded speckit.* commands) for existing presets.

        Args:
            preset_name: The preset ID to refresh commands for
        """
        manifest = self.get_pack(preset_name)
        if manifest is None:
            return
        dest_dir = self.presets_dir / manifest.id
        registered_commands = self._register_commands(manifest, dest_dir)
        self.registry.update(preset_name, {"registered_commands": registered_commands})
