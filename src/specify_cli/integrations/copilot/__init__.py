...
class CopilotIntegration(IntegrationBase):
    ...
    def is_skills_mode(
        ...
        if opts.get("commands"):
            return False
        # Fork: skills/commands may use "spec-" or "speckit-" prefix depending
        # on whether presets are active (aliases rename speckit.* → spec.*).
        try:
            from ..base import _get_command_prefix
            _prefixes = (_get_command_prefix(), "speckit")
        except ImportError:
            _prefixes = ("speckit",)
        if project_root is not None:
            ...
            if manifest_path.is_file():
                ...
                if manifest_files is not None and any(
                    any(
                        path.startswith(f".github/skills/{p}-")
                ...
                if manifest_files is not None and any(
                    any(
                        path.startswith(f".github/agents/{p}.")
            ...
            has_managed_commands = any(
                any(
                    (
                        github_dir
                        / "agents"
                        / f"{p}.{command}.agent.md"
                    ).is_file()
                    or (
                        github_dir
                        / "prompts"
                        / f"{p}.{command}.prompt.md"
                    ).is_file()
                    for p in _prefixes
                )
                for command in _COPILOT_CORE_COMMANDS
    ...
    def build_command_invocation(self, command_name: str, args: str = "") -> str:
        ...
        if self._skills_mode:
            ...
            try:
                ...
                resolved = resolve_command_alias(canonical)
            except Exception:
