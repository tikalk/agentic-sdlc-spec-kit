"""Fork leaf module: agent runtime hooks for integrations.
...
This module is a leaf (imports nothing from other ``_*_fork`` modules) per FORK.md.
"""
...
def validate_runtime_hooks(data: dict[str, Any]) -> None:
    ...
    Called from ``ExtensionManifest._validate()`` via a fork callout.
