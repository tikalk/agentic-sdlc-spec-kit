...
Fork note: the fork's bash scripts have enhanced features (worktree isolation,
issue tokens, number_padding) that upstream's Python ports do not yet replicate.
Parity tests are skipped on the fork until the Python scripts are extended.
"""
...
# Fork: skip parity tests — fork's bash scripts produce enhanced output
# (ISOLATION_MODE, WORKTREE_PATH, MANIFEST_PATH) that upstream's basic Python
# scripts don't. See FORK.md "Git Extension Worktree & Task-Execution Feature".
try:
    from specify_cli import PKG_NAMES
    if any("agentic-sdlc" in pkg for pkg in PKG_NAMES):
        import pytest
        pytest.skip(
            "Fork's bash git scripts have worktree/isolation features that "
            "upstream's Python ports don't yet replicate; parity tests skip "
            "until Python scripts are extended (planned for future adlc release)",
