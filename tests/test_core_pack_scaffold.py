...
_RELEASE_SCRIPT = (
    _REPO_ROOT / ".github" / "workflows" / "scripts" / "create-release-packages.sh"
)
...
def _run_release_script(
    agent: str, script_type: str, bash: str, output_dir: Path
) -> Path:
    ...
    result = subprocess.run(
        [bash, str(_RELEASE_SCRIPT), "agentic-sdlc-v0.0.0"],
        capture_output=True,
        text=True,
        cwd=str(_REPO_ROOT),
        env=env,
    ...
    zip_pattern = (
        f"agentic-sdlc-spec-kit-template-{agent}-{script_type}-agentic-sdlc-v0.0.0.zip"
    )
...
def test_scaffold_creates_specify_scripts(agent, scaffolded_sh):
    ...
    assert any(scripts_dir.iterdir()), (
        f".specify/scripts/bash/ is empty for agent '{agent}'"
    )
...
@pytest.mark.parametrize("agent", _TESTABLE_AGENTS)
def test_parity_bundled_vs_release_script(
    agent, script_type, scaffolded_sh, scaffolded_ps, release_script_trees
):
...
def test_pyproject_force_include_covers_all_templates():
    ...
    repo_template_files = sorted(f.name for f in templates_dir.iterdir() if f.is_file())
    ...
    missing = [
        name for name in repo_template_files if f"templates/{name}" not in force_include
    ]
    assert not missing, (
