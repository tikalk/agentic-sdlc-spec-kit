...
def _http_error(code: int, message: str = "error") -> urllib.error.HTTPError:
    return urllib.error.HTTPError(
        url="https://api.github.com/repos/tikalk/agentic-sdlc-spec-kit/releases/latest",
        code=code,
        msg=message,
...
class TestUserStory1:
    def test_newer_available_prints_update_and_install_command(self):
        ...
        assert "git+https://github.com/tikalk/agentic-sdlc-spec-kit.git@" in output
    ...
    def test_unknown_installed_still_prints_latest_and_reinstall(self):
        ...
        assert "0.7.4" in output
        # Fork and upstream may render different repo URLs here; keep the
        # behavioral assertion focused on the upgrade guidance itself.
        assert "git+https://github.com/" in output
    ...
    def test_unknown_installed_uses_placeholder_when_latest_tag_is_invalid(self):
        ...
        assert "git+https://github.com/tikalk/agentic-sdlc-spec-kit.git@vX.Y.Z" in output
