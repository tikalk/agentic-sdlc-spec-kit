...
4. **Check checklists status** (if FEATURE_DIR/checklists/ exists):
   ...
   - `checklists/requirements.md` is the built-in spec-quality checklist maintained by `/spec.specify` and `/spec.clarify`; custom checklists generated by `/spec.checklist` are reviewer-owned requirements-quality review artifacts
   ...
   - Count: total items (`- [ ]` / `- [X]` / `- [x]`), checked (`- [X]` / `- [x]`), unchecked (`- [ ]`)
   - Create status table:
   ...
   - **If all checklists are checked**: Display the table and automatically proceed to step 5
